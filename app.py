...
st.markdown("""
    ...
    /* Arama Barı Stili (Metin beyaz, kenarlık kırmızı) */
    .stTextInput input {
...
if not movie_name:
    st.markdown("""
        <div class='opening-screen'>
            ...
            <p class='opening-text'>Favori filmini ara ve gerçek puanını keşfet.</p>
        </div>
    """, unsafe_allow_html=True)
...
# Film Aranmışsa
    ...
    if o_data.get("Response") == "True" and t_search_data.get("results"):
        ...
        if "Horror" not in genre:
            st.error("🛑 Uyarı: The Hartim Curve sadece korku janrı için tasarlanmış özel bir algoritmadır. Lütfen bir korku filmi girin.")
        else:
            ...
            if H > 0 and E > 0:
                fark = H - E
                if fark >= 2.5:
                    B = (H * 0.90) + (E * 0.10) # Ağır Linç Kalkanı
                elif fark >= 1.5:
                    B = (H * 0.75) + (E * 0.25) # Hafif Linç Kalkanı
                else:
                    B = (H * 0.50) + (E * 0.50) # Standart Denge
            elif H > 0:
                B = H
            elif E > 0:
                B = E
            ...
            if B > 0:
                ...
                with col2:
                    ...
                    # Final Kutu (Rozetler kaldırıldı, saf estetik)
                    st.markdown(f"""
